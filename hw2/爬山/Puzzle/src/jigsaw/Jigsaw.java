...
//        int s = JackMostow(jNode);
    ...
    public boolean simulated() {
        ...
        int trial = 0;
        this.estimateValue(this.currentJNode);
        while(temperature > 0.000001) {  
            Vector<JigsawNode> followJNodes = new Vector<JigsawNode>();  
            JigsawNode tempJNode;  
            for(int i=0; i<4; i++){  //获取后续节点
                tempJNode = new JigsawNode(this.currentJNode);  
                if(tempJNode.move(i))  
                    followJNodes.addElement(tempJNode);  
            }
            int i = (new Random().nextInt(followJNodes.size()));
            JigsawNode nextJNode = followJNodes.get(i);
            this.estimateValue(nextJNode);
            int e1 = nextJNode.getEstimatedValue() - this.currentJNode.getEstimatedValue();
            if (e1 < 0) {                    // 下一状态优于当前状态
                this.currentJNode = nextJNode;
                trial++;
            ...
            if (this.currentJNode.equals(this.endJNode)) {   // 成功找到解
                isCompleted = true;
                return true;
            }
        }
    ...
    public boolean climbHill() {
        ...
        while(trial <= 500) {
            ...
            tempJNode = followJNodes.get(0);
            for(int i = 1; i < followJNodes.size(); i++) {//选取估计值最小的
                if(followJNodes.get(i).getEstimatedValue() < tempJNode.getEstimatedValue()) {
                    tempJNode = followJNodes.get(i);
                }
            }
            if(tempJNode.getEstimatedValue() <= this.currentJNode.getEstimatedValue()) {
                trial++;
                currentJNode = tempJNode;
            } else {//没有合适的后续节点则失败
                return false;
            }
            if (this.currentJNode.equals(this.endJNode)) {   // 成功找到解
    ...
    /**
     * 首选爬山法 随机选取第一个估计值比当前低的后续节点
     * @return 成功或者失败
     */
    public boolean firstChose() {
        int trial = 0;
        this.estimateValue(this.currentJNode);
        while(trial <= 500) {
            Vector<JigsawNode> followJNodes = new Vector<JigsawNode>();  
            ...
            for(int i = 0; i < followJNodes.size(); i++) {
                if(followJNodes.get(i).getEstimatedValue() > this.currentJNode.getEstimatedValue()) {
                    tempJNode = followJNodes.remove(i);//去掉所有比当前节点估计值大的后续节点
                    i--;
                }
            }
            if(followJNodes.size() == 0) {
                return false;//可选后续节点为空
            } else {//从比当前小的后续节点中随机选择
                int i = new Random().nextInt(followJNodes.size());
                currentJNode = followJNodes.get(i);
                trial++;
            }
            if (this.currentJNode.equals(this.endJNode)) {//成功找到解
                isCompleted = true;
